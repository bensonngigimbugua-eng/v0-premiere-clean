...
export class DerivWebSocket {
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log("[v0] 🚀 Initiating Deriv API connection...")
        ...
        setTimeout(() => {
          if (this.ws && this.ws.readyState !== WebSocket.OPEN) {
            console.error("[v0] ⏱️ Connection timeout - retrying...")
            this.ws.close()
  ...
  subscribeTicks(symbol: string): void {
    if (!this.isConnected()) {
      console.warn("[v0] ⚠️ Cannot subscribe - not connected, attempting reconnection...")
  ...
  unsubscribeTicks(): void {
    if (!this.isConnected()) {
      console.warn("[v0] ⚠️ Cannot unsubscribe: not connected")
      return
    }
    this.send({
      forget_all: "ticks",
